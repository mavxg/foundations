define('friar', function(module, exports, require) {
...
var isDOMAttribute = {
	...
	href: PROPERTY,
	spellcheck: PROPERTY,
	height: true, //setAttribute
	width: true, //setAttribute
	title: true,
	type: true,
	x:true,
	y:true,
	...
	contentEditable: function(action, node, value) {
		if (action==='delete') 
	...
	if (this.type === DOM.text.type) {
		...
		for (var prop in this.props) {
			if (isDOMAttribute[prop]) {
				...
				//	this.removeListener(prop, this.__listeners[prop]);
				this.addListener(prop, this.props[prop]);
				//this.__listeners[prop] = this.props[prop];
			}
		}
		var node = this.node;
